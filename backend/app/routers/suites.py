...
Fix applied
───────────
Replaced all deprecated SQLModel `.from_orm()` calls with `.model_validate()`
(Pydantic v2 / SQLModel ≥ 0.0.16).  from_orm() was silently falling back to
dict-mode on newer versions and would raise AttributeError on strict builds.
"""
...
def get_suite(suite_id: int, session: Session = Depends(get_session)):
    ...
    return {
        ...
        "test_cases": [TestCaseRead.model_validate(c) for c in cases],
    }
...
def update_suite(
    ...
    # model_dump(exclude_unset=True) is the Pydantic v2 equivalent of
    # .dict(exclude_unset=True) — only fields the caller actually sent are patched.
...
def add_test_case(
    ...
    db_case = TestCase.model_validate(case)
